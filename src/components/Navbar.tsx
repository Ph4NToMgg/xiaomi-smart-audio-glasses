  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-40 transition-all duration-300 ${
        isScrolled
          ? 'bg-[#050507]/80 backdrop-blur-xl border-b border-zinc-800/80 py-3'
          : 'bg-transparent py-5'
      }`}
    >
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-8 text-xs font-mono">
          {navLinks.map((link) => (
            <a
              key={link.name}
              href={link.href}
              className="text-zinc-400 hover:text-yellow-400 transition-colors py-1 relative group"
            >
              {link.name}
        ...
        <div className="hidden sm:flex items-center gap-4">
          <button
            ...
            className="p-2 rounded-xl bg-zinc-900/80 border border-zinc-800 text-zinc-400 hover:text-yellow-400 hover:border-yellow-500/40 transition-all"
          ...
          <a
            href="#buy"
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-yellow-400 hover:bg-yellow-300 text-black font-bold text-xs tracking-wide uppercase transition-all transform hover:scale-105 shadow-[0_0_20px_rgba(255,199,0,0.3)] font-heading"
          >
        ...
        <div className="md:hidden bg-zinc-950/95 border-b border-zinc-800 px-6 py-6 space-y-4">
          {navLinks.map((link) => (
            <a
              ...
              className="block text-sm font-mono text-zinc-300 hover:text-yellow-400"
            >
              {link.name}
            </a>
          ))}
          <div className="pt-4 border-t border-zinc-900 flex items-center justify-between">
            <a
              href="#buy"
              onClick={() => setMobileMenuOpen(false)}
              className="w-full text-center px-4 py-3 rounded-xl bg-yellow-400 text-black font-bold text-xs uppercase font-heading"
            >
              Pre-Order Now ($199)
            </a>
          </div>
